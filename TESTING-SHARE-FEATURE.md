...
### Create Share Link
```bash
curl -X POST http://localhost:3000/api/workspace/YOUR_WORKSPACE_ID/share \
...
### Get Share Preview
```bash
curl http://localhost:3000/api/share/YOUR_SHARE_TOKEN
```

### Clone Workspace
```bash
curl -X POST http://localhost:3000/api/share/YOUR_SHARE_TOKEN/clone \
  -H "Content-Type: application/json" \
  -b cookies.txt \
...
### Revoke Share Link
```bash
curl -X DELETE http://localhost:3000/api/workspace/YOUR_WORKSPACE_ID/share \
  -b cookies.txt
```
